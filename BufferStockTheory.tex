...
% Create a command with syntax: \JY{Original version}{Proposed Modification by JY} % PrivateMsg
%\provideboolean{JY} % Jiaxiong Yao % PrivateMsg
%\setboolean{JY}{true} % PrivateMsg
%\newcommand{\JY}[2]{\ifthenelse{\boolean{JY}}{\marginpar{\large JY}\{{#1}\} $\rightarrow$ \{{#2}\}}{#2}} % PrivateMsg
...
\maketitleWithForcedDate{October 22, 2018}
...
%  \begin{verbatimwrite}{\EqDir/veqn.tex}
\begin{eqnarray}
...
\end{eqnarray}
which we call the Perfect Foresight Finite Value of Autarky Condition, PF-FVAC, because it guarantees that a consumer who always spends all his permanent income will have finite value (the consumer has `finite autarky value').  Note that the version of the PF-FVAC in \eqref{eq:PFFVACv2} implies the PF-GIC $\PatPGro < 1$ whenever \cancel{PF-FHWC} $\Rfree < \PGro$ holds.  So, if \cancel{PF-FHWC} holds, value for any finite $\mRat$ will be the sum of two finite numbers: The component due to the unconstrained consumption choice made over the finite horizon leading up to $\bRat_{t+n} = 0$, and the finite component due to the value of consuming all income thereafter.  The consumer's value function is therefore nondegenerate.
...
\begin{figure}[h]
\centerline{\includegraphics[width=6in]{\FigDir/SimRevolution}}
...
%\input{\ApndxDir/ApndxLiqConstr.tex}
...
%\input{\ApndxDir/ApndxSolnMethEndogGpts.tex}
\subfile{\ApndxDir/ApndxSolnMethEndogGpts}
%\input{\ApndxDir/ApndxSolnMethTermFunc.tex}
